...
Core.run = function() {
  ...
  if(CliOptions.command === "wizard") {
    PulseAudioWizard.start(CliOptions.wizardType, this, DataManager);
  }
  // Switch to sink
  ...
  if(switchType.default) {
    ...
    proc.stdout.on("data", (data) => {
      ...
      if(str.match(/Sink (?:.+) does not exist./)) {
        this.displayWarning(WARN_NOSINKDEFAULT);
        hasResult = true;
      }
    });
  ...
  if(switchType.mute) {
    ...
    proc.stdout.on("data", (data) => {
      ...
      if(str.trim() === "No sink found by this name or index.") {
        this.displayWarning(WARN_NOSINKUNMUTE);
        hasResult = true;
      }
    });
    // Event: Close (if hasResult is false, it probably went alright)
...
Core.disableSink = function(sinkObj, switchType) {
  ...
  if(switchType.mute) {
    ...
    proc.stdout.on("data", (data) => {
      let str = data.toString();
      if(str.trim() === "No sink found by this name or index.") {
        this.displayWarning(WARN_NOSINKMUTE, sinkObj);
      }
    });
  }
};
